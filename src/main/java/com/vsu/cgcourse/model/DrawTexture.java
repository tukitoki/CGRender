    ...
    public static void drawPixels(Mesh mesh) {
        ...
        for (int i = 0; i < mesh.getPolygons().getPolygonTextureVertexIndices().size(); i++) {
            ...
            if (findPointOppositeX(v0, v1, v2) == null){
                Vector2 v3 = findForthPoint(v0, v1, v2);
                for (float y = vectors.get(2).getY(); Math.abs(y - v3.getY()) > 1E-5; y--) {
                    for (float x = getXFuncLine(vectors.get(2), getOppositePoint(vectors.get(2), v3, v3.getY()), y);
                         x < getXFuncLine(vectors.get(2), v3, y); x++) {
                        pixelWriter.setColor((int)x, (int)y, Color.PURPLE);
                    }
                }
            } else {
                Vector2 vectorMinY = getVectorMinY(v0, v1, v2);
                for (float y = vectors.get(2).getY(); y < vectorMinY.getY(); y++) {
                    for (float x = getXFuncLine(vectors.get(2), ); x < getXFuncLine(); x++) {
